...
2. Locate the project's `cross_matrix_tasks.md`. It is the only authority for the 9 industries and 44 occupations. Record its path and SHA-256; without it, collect leads only, never formal candidates.
...
- Its industry and occupation exactly match the matrix, with occupational evidence rated A, B, or C. A is direct job-title evidence; B is a documented department, document-type, and duty chain; C is a documented de-identified scenario mapping. Do not admit U (insufficient evidence).
